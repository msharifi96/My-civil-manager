...
with tabs[3]:
    ...
    with cl:
        ...
        if mode_l == "افزودن":
            ...
            all_l = pd.read_sql("SELECT * FROM locations WHERE level=? AND p_type=?", conn, params=(lvl, m_sec))
            if not all_l.empty:
                tg = st.selectbox("انتخاب مورد برای حذف نهایی:", all_l['name'].tolist(), key="l_del_tg")
                tid = all_l[all_l['name']==tg]['id'].values[0]
                with st.popover("⚠️ تایید حذف زنجیره‌ای", use_container_width=True):
                    st.error(f"با حذف {tg} تمام زیرمجموعه‌ها و پروژه‌ها پاک می‌شوند.")
                    if st.button("تایید حذف قطعی"):
                        if lvl == "استان":
                            ...
                            for vid in v_ids:
                                ...
                                c.execute("DELETE FROM projects WHERE loc_id=?", (int(vid),))
                                c.execute("DELETE FROM locations WHERE id=?", (int(vid),))
                        else: # حذف شهر یا روستا + پروژه‌های آن
    ...
    with cr:
        ...
        if mode_p == "افزودن":
            ...
            if not all_p.empty:
                all_p['disp'] = all_p.apply(lambda x: f"ق: {x['contract_no']} - {x['name']}", axis=1)
                tg = st.selectbox("انتخاب پروژه برای ویرایش:", all_p['disp'].tolist(), key="p_edit_sel")
                pid = all_p[all_p['disp']==tg]['id'].values[0]
                p_d = all_p[all_p['id']==pid].iloc[0]
                
                v_l = pd.read_sql("SELECT * FROM locations WHERE level='شهر یا روستا' AND p_type=?", conn, params=(m_sec,))
                cur_l_n = v_l[v_l['id']==p_d['loc_id']]['name'].values[0] if p_d['loc_id'] in v_l['id'].values else v_l['name'].tolist()[0]
                
                new_l = st.selectbox("محل:", v_l['name'].tolist(), index=v_l['name'].tolist().index(cur_l_n))
            ...
            if not all_p.empty:
                ...
                with st.popover("🗑️ تایید حذف پروژه", use_container_width=True):
                    if st.button("بله، پروژه و فایل‌ها حذف شوند"):
