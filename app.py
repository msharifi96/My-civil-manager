...
# ۱. اتصال به دیتابیس (داده‌های شما محفوظ می‌ماند)
...
# استایل برای راست‌چین کردن و حذف مربع دور آیکون‌ها
st.markdown("""
    <style>
    .stApp { direction: rtl; text-align: right; }
    
    /* حذف کادر دور دکمه‌های آیکونی */
    ...
    /* اصلاح نمایش تب‌ها برای پایداری */
                    ...
                    for _, fl in files.iterrows():
                        ...
                        with c_icons:
                            ...
                            if i2.button("🔗", key=f"lnk_{fl['id']}"):
                                b64 = base64.b64encode(fl['file_blob']).decode(); st.toast("لینک تولید شد")
...
with tabs[3]:
    ...
    m_sec = st.radio("بخش:", ["نظارتی 🛡️", "شخصی 👷"], horizontal=True, key="m_set")
    st.divider()
    cl, cr = st.columns(2) # برگشت به حالت دو ستونه مشابه نسخه ۲۶
    with cl:
    ...
    with cr:
        ...
        if not all_projs.empty:
            ...
            if st.button("ایجاد پوشه"):
                pid = all_projs[all_projs['name']==spj]['
